...
def main():
    ...
    parser = argparse.ArgumentParser(description="Classifie les textes web B2B/B2C.")
    project_root = Path(__file__).resolve().parents[1]
    parser.add_argument("--companies", default=project_root / "data" / "company_urls.xlsx")
    parser.add_argument("--texts", default=project_root / "data" / "cleaned_text")
    parser.add_argument("--output", default=project_root / "results" / "classification_results.xlsx")
    args = parser.parse_args()

    grouper = SimpleGrouper()

    # 1. Excel laden
    grouper.load_companies(args.companies)

    # 2. Ordner wählen
    grouper.process_files(args.texts)

    # 4. Export
    Path(args.output).parent.mkdir(parents=True, exist_ok=True)
    grouper.export(args.output)
