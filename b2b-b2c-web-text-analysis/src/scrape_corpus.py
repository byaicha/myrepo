...
INPUT_EXCEL = PROJECT_ROOT / "data" / "company_urls.xlsx"
EXCEL_SHEET = "Unternehmen"  # Name des Blatts

# Ordner für Outputs
RAW_HTML_DIR = PROJECT_ROOT / "data" / "raw_html"
CLEAN_TEXT_DIR = PROJECT_ROOT / "data" / "cleaned_text"
...
def setup_logging():
    ...
    log_filename = LOG_DIR / f"scraping_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
def save_data(company_name, page_type, html_content, clean_text):
    ...
    text_path = CLEAN_TEXT_DIR / text_filename
...
def main():
    ...
    metadata_path = LOG_DIR / 'scraping_metadata.json'
    import json
